...
def main():
    ...
    efficient_test = True  # Use temp files to save memory (paths stored in .pkl, actual files preserved)
    ...
    if rank == 0:
        ...
        # Check if results contains string BEFORE clearing memory
        if isinstance(outputs[0], str):
            # DON'T delete temp files - we want to keep them for later evaluation
            # dataset.remove_tmp_files(outputs)
            print(f'✓ Temp prediction files preserved at: {outputs[0].split("/")[0]}')
            print(f'  Total predictions: {len(outputs)} files')
        
        
        # Clear outputs from memory after everything is done
        del outputs
        gc.collect()
