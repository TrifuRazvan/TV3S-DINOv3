...
#        print(label.shape)
    ...
    def add_batch(self, gt_image, pre_image):
        ...
        try: 
            ...
            print(np.unique(gt_image), np.unique(pre_image))
            self.confusion_matrix += self._generate_matrix(gt_image, pre_image)
...
## Pred as user input
Pred=sys.argv[1]
DIR = sys.argv[2] if len(sys.argv) > 2 else '/workspace/TV3S/data/vspw/VSPW_480p'
...
for video in videolist:
    ...
    try:
        images = sorted(os.listdir(os.path.join(Pred,video)))
